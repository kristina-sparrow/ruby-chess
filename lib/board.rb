...
class Board
  ...
  def move_piece(start_pos, end_pos)
    ...
    unless piece.safe_moves.include?(end_pos)
      raise InvalidMoveError, "End position (#{end_pos}) not in available moves: #{piece.safe_moves}"
    end
    raise InvalidMoveError, 'End position not in bounds' unless in_bounds?(end_pos)
